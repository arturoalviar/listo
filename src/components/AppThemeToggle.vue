<template>
  <div
    class="listo-theme-toggle-wrapper flex flex-col justify-center items-center w-full"
  >
    <button
      class="listo-theme-toggle cursor-pointer flex flex-col justify-center items-center p-4 w-full focus:outline-none"
      :aria-label="'Toggle next theme' + nextTheme"
      @click="toggleNextMode"
    >
      <SunIcon v-if="selectedMode === 'light'" class="listo-icon" />
      <svg
        v-if="selectedMode === 'sakura'"
      ...
      <MoonIcon v-if="selectedMode === 'dark'" class="listo-icon" />
    </button>
  </div>
</template>
...
@Component({
  name: 'app-theme-toggle',
  components: {
    SunIcon,
    MoonIcon,
  },
})
export default class AppThemeToggle extends mixins(themeToggle) {}
</script>
